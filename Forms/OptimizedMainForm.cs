    ...
    public partial class OptimizedMainForm : Form
    {
        ...
        private async Task RefreshS3Files()
        {
            try
            {
                ...
                UpdateS3TreeView();
                UpdateStatus($"Loaded {_s3Files.Count} files from S3");
            }
        ...
        private void UpdateS3TreeView()
        {
            ...
            foreach (var file in _s3Files.OrderBy(f => f.Key))
            {
                var parts = file.Key.Split('/');
                TreeNodeCollection currentNodes = _s3TreeView.Nodes;
                string currentPath = "";
                ...
                for (int i = 0; i < parts.Length; i++)
                {
                    ...
                    bool isFile = i == parts.Length - 1 && !file.IsDirectory;
                    
                    TreeNode? node = null;
                    foreach (TreeNode n in currentNodes)
                    {
                        if (n.Text.StartsWith(parts[i]))
                        {
                            node = n;
                            break;
                        }
                    ...
                    if (node == null)
                    {
                        string displayText = isFile 
                            ? $"📄 {parts[i]} ({FormatFileSize(file.Size)})"
                            : $"📁 {parts[i]}";
        ...
        private async void SyncButton_Click(object? sender, EventArgs e)
        {
            using (var dialog = new FolderBrowserDialog())
            {
                ...
                {
                    _progressBar!.Value = (int)p.PercentComplete;
                ...
                try
                {
                    ...
                    var result = await _s3Service.SyncS3ToLocalAsync(
                        dialog.SelectedPath, "",
                        _currentUser.Role, progress, _currentOperation.Token);
                    
                    var message = $"Sync completed:\n" +
                                 $"Downloaded: {result.DownloadedCount} files\n" +
