...
namespace S3FileManager.Services
{
    ...
    public class OptimizedS3Service : IDisposable
    {
        ...
        public async Task<List<S3FileItem>> ListFilesAsync(UserRole userRole, string prefix = "",
            CancellationToken cancellationToken = default, bool delimited = false)
        {
            ...
            do
            {
                ...
                if (delimited)
                {
                    ...
                    {
                        files.Add(new S3FileItem { Key = commonPrefix, IsDirectory = true });
                    }
                }
                ...
                {
                    if (item != null && CanUserAccessFile(userRole, item))
                    {
                        files.Add(item);
                    }
                }
        ...
        public async Task<bool> IsVersioningEnabledAsync()
        {
            try
            {
                ...
                var response = await _s3Client.GetBucketVersioningAsync(request);
                return response.VersioningConfig.Status == VersionStatus.Enabled;
            }
            catch (AmazonS3Exception ex)
            {
                // Handle cases where user may not have GetBucketVersioning permissions
                Console.WriteLine($"Could not check bucket versioning: {ex.Message}");
                return false;
            }
        }
                ...
                .Select(async _ =>
                {
                    await foreach (var (filePath, key) in channel.Reader.ReadAllAsync(cancellationToken))
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            Debug.WriteLine($"Failed to upload {filePath}: {ex.Message}");
                        }
                    }
                });
        ...
        public async Task<SyncResult> SyncLocalToS3Async(string localPath, string s3Prefix,
            UserRole userRole, IProgress<SyncProgress>? progress = null,
            CancellationToken cancellationToken = default)
        {
            var result = new SyncResult();
            ...
            syncProgress.Status = "Scanning local files...";
            progress?.Report(syncProgress);
            var localFiles = GetLocalFilesRecursive(localPath, localPath);
            var localFileDict = localFiles;

            // Get S3 files
            syncProgress.Status = "Listing S3 files...";
            progress?.Report(syncProgress);
            var s3Files = await ListFilesAsync(userRole, s3Prefix, cancellationToken);
            var s3FileDict = s3Files.Where(f => !f.IsDirectory).ToDictionary(f => f.Key, f => f);

            var toUpload = new List<FileInfo>();

            foreach (var localFile in localFileDict)
            {
                var s3Key = string.IsNullOrEmpty(s3Prefix)
                    ? localFile.Key
                    : Path.Combine(s3Prefix, localFile.Key).Replace('\\', '/');

                if (s3FileDict.TryGetValue(s3Key, out var s3File))
                {
                    // A tolerance of 2 seconds is recommended when comparing S3 modification times
                    if (localFile.Value.LastWriteTimeUtc > s3File.LastModified.ToUniversalTime().AddSeconds(2) ||
                        localFile.Value.Length != s3File.Size)
                    {
                        toUpload.Add(localFile.Value);
                    }
                    else
                    {
                        result.SkippedCount++;
            ...
            if (toUpload.Count > 0)
            {
                syncProgress.Status = "Uploading files...";
                syncProgress.TotalFiles = toUpload.Count;
                progress?.Report(syncProgress);

                var uploadChannel = Channel.CreateUnbounded<FileInfo>();
                ...
                var producerTask = Task.Run(async () =>
                {
                    foreach (var file in toUpload)
                    {
                        ...
                        await uploadChannel.Writer.WriteAsync(file, cancellationToken);
                    }
                ...
                var consumerTasks = Enumerable.Range(0, _performanceConfig.MaxConcurrentUploads)
                    .Select(async _ =>
                    {
                        await foreach (var fileInfo in uploadChannel.Reader.ReadAllAsync(cancellationToken))
                        {
                            try
                            {
                                var relativePath = Path.GetRelativePath(localPath, fileInfo.FullName).Replace('\\', '/');
                                var s3Key = string.IsNullOrEmpty(s3Prefix)
                                    ? relativePath
                                    : Path.Combine(s3Prefix, relativePath).Replace('\\', '/');

                                await UploadFileAsync(fileInfo.FullName, s3Key, new List<UserRole> { userRole }, null, cancellationToken);

                                result.UploadedCount++;
                                syncProgress.ProcessedFiles = result.UploadedCount;
                                progress?.Report(syncProgress);
                            }
                            catch (Exception ex)
                            {
                                result.Errors.Add($"Failed to upload {fileInfo.Name}: {ex.Message}");
                            }
                        }
                    });
        ...
        private Dictionary<string, FileInfo> GetLocalFilesRecursive(string path, string rootPath)
        {
            ...
            {
                var subFiles = GetLocalFilesRecursive(dir.FullName, rootPath);
